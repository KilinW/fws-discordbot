...
class ChatThread:
    def __init__(self, thread: discord.Thread, db: ChatDB):
        ...
        self.agent: LangChainAgent = LangChainAgent(self.db)
    ...
    async def reload(self) -> None:
        ...
        if len(self.msg_history) == 1:  # No regular message
            welcome_msg = await self.thread.send("Welcome. Bitch!")
    ...
    async def valid_thread_init(
        ...
        if len(self.msg_history) > 3:  # Only invite, welcome, first question.
            return
